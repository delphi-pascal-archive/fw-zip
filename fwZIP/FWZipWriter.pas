...
type
  ...
  private
    FOwner: TFWZipWriter;
    FComment: AnsiString;            // коментарий к элементу
    FFilePath: AnsiString;           // путь к внешнему файлу (или см. TFWZipWriterItemEx.Data)
    FFileName: AnsiString;           // имя элемента в архиве - не может быть пустым
    FPassword: AnsiString;           // пароль
    ...
    procedure SetString(const Index: Integer; const Value: AnsiString);
  protected
  ...
  public
    ...
    property Comment: AnsiString index 0 read FComment write SetString;
    property FilePath: AnsiString index 1 read FFilePath write SetString;
    property FileName: AnsiString index 2 read FFileName write SetString;
    property Password: AnsiString index 3 read FPassword write SetString;
  ...
  protected
    ...
    procedure SaveString(Stream: TStream; const Value: AnsiString);
  ...
  public
    ...
    function AddFolder(const RelatePath, Path, Mask: string;
      SubFolders: Boolean = True): Integer;
    procedure BuildZip(const ZipFilePath: string); overload;
    procedure BuildZip(Stream: TStream); overload;
    function Count: Integer;
    procedure DeleteItem(Index: Integer);
    property Item[Index: Integer]: TFWZipWriterItem read GetItem; default;
    property Commets: string read FComment write FComment;
    ...
    case Index of
      ...
      2:
        if Length(Value) >= MAX_PATH then
          raise Exception.Create('Слишком длинный путь.')
        else
          FFileName := Value;
      3: FPassword := Value;
    end;
...
begin
  ...
  // и мы смогли снять его аттрибуты
  if GetFileAttributesEx(PChar(FilePath),
    GetFileExInfoStandard, @Attributes) then
    Result := AddFile(FilePath, Attributes, FileName);
end;
...
begin
  ...
  if FileName = '' then
    InitFileName := ExtractFileName(ExcludeTrailingPathDelimiter(FilePath))
  else
    InitFileName := CheckFileNameSlashes(FileName);

  Item := TFWZipWriterItem.Create(Self, FilePath, Attributes, InitFileName);
...
//  В качестве результата возвращает количество успешно добавленных элементов.
// =============================================================================
function TFWZipWriter.AddFolder(const RelatePath, Path, Mask: string;
  SubFolders: Boolean): Integer;
var
  SR: TSearchRec;
  TrailingPath, TrailingRelatePath, ResultMask: string;
  Attributes: TWin32FileAttributeData;
begin
  Result := 0;
  // обычное рекурсивное сканирование папки
  // единственный нюанс - параметр RelatePath
  // в котором передается путь к файлу или папке относительно корневой папки
  if RelatePath = '' then
    TrailingRelatePath := RelatePath
  else
  begin
    TrailingRelatePath := IncludeTrailingPathDelimiter(RelatePath);
    if AddFile(Path, TrailingRelatePath) >= 0 then
      Inc(Result);
  end;
  TrailingPath := IncludeTrailingPathDelimiter(Path);
  if Mask = '' then
    ResultMask := '*.*'
  else
    ResultMask := Mask;
  ...
  try
    repeat
      ...
      begin
        if SubFolders then
          AddFolder(TrailingRelatePath + SR.Name,
      ...
      begin
        ...
        if AddFile(TrailingPath + SR.Name, Attributes,
          TrailingRelatePath + SR.Name) >= 0 then
          Inc(Result);
      end;
    until FindNext(SR) <> 0;
  finally
...
// =============================================================================
procedure TFWZipWriter.BuildZip(Stream: TStream);
var
  I: Integer;
...
// =============================================================================
procedure TFWZipWriter.BuildZip(const ZipFilePath: string);
var
  ZIP: TFileStream;
begin
    ...
    if FTotalSizeCount = 0 then
      ...
      TotalProgress :=
        Round((FTotalProcessedCount + FCompressedStream.Position) /
          (FTotalSizeCount / 100));
    FOnProgress(Self, string(Item[FProcessedItemIndex].FileName),
...
begin
  ...
  FDefaultPassword := AnsiString(DefaultPassword);
...
begin
  ...
  // Исключем из общего количества элементов архива количество исключений
  ...
  if eocd.ZipfileCommentLength > 0 then
    SaveString(Stream, AnsiString(FComment));
end;
...
begin
  ...
  if not CurrentItem.NeedDescriptor then
    ...
    begin
      ...
      if CurrentItem.FilePath <> '' then
        FCD[Index].Header.Crc32 := FileCRC32(string(CurrentItem.FilePath))
      else
        FCD[Index].Header.Crc32 := CRC32Calc(CurrentItem.Data.Memory, CurrentItem.Data.Size);
    end;
  ...
  FCD[Index].Header.ExternalFileAttributes := CurrentItem.Attributes.dwFileAttributes;
    ...
    try
      ...
      case FCD[Index].Header.CompressionMethod of
        ...
        begin
          ...
          begin
            F := TFileStream.Create(string(CurrentItem.FilePath), fmOpenRead);
            try
              FCD[Index].Header.Crc32 :=
...
// =============================================================================
procedure TFWZipWriter.SaveString(Stream: TStream; const Value: AnsiString);
var
