...
type
  ...
  TExtractResult = (erError, erDone, erNeedPassword);
  ...
  public
    function Extract(const Path, Password: string): TExtractResult;
    ...
    property Comment: AnsiString index 0 read GetString;
    property IsFolder: Boolean read FIsFolder;
    property FileName: AnsiString index 1 read GetString;
...
begin
  if Assigned(FOnProgress) then
    if Sender = nil then
      FOnProgress(Self, string(FileName), FTotalExtracted, UncompressedSize)
    else
    begin
...
begin
  ...
  FullPath := StringReplace(
    IncludeTrailingPathDelimiter(Path) + string(FFileHeader.FileName),
    '/', '\', [rfReplaceAll]);
  ...
  try
    ...
    begin
      FileDate := FFileHeader.Header.LastModFileTimeTime +
...
//
// =============================================================================
function TFWZipReaderItem.GetString(const Index: Integer): AnsiString;
begin
  case Index of
...
// =============================================================================
procedure TFWZipReaderItem.LoadStringValue(var Value: AnsiString;
  nSize: Cardinal);
begin
  if Integer(nSize) > 0 then
  begin
    SetLength(Value, nSize);
    FOwner.ZIPStream.ReadBuffer(Value[1], nSize);
...
begin
  ...
  begin
    ...
    FOnProgress(Self, FileName, Percent, TotalPercent);
  end;
end;
...
begin
  ...
  if DiskFree(Byte(AnsiString(UpperCase(Path))[1]) - $40) < FTotalSizeCount then
    raise Exception.Create('Недостаточно места на диске.');

  for I := 0 to Count - 1 do
  begin
    CurrentItem := Item[I];
    OldEvent := CurrentItem.OnProgress;
    try
      ...
      try
        ...
        begin
          ...
          begin
            ...
            if ExtractResult = erDone then Break;
          end;
          ...
          if ExtractResult = erNeedPassword then
            ...
            begin
              ...
              begin
                Password := '';
                FOnNeedPwd(Self, string(CurrentItem.FileName),
      ...
      CurrentItem.OnProgress := OldEvent;
    end;
  end;
end;
...
begin
  Result := -1;
  for I := 0 to Count - 1 do
    if AnsiCompareText(string(Item[I].FileName), FileName) = 0 then
    begin
      Result := I;
      Break;
    end;
